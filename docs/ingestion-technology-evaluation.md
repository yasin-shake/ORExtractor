...
Three ideas are genuinely worth testing, but they should not be adopted as one
large rewrite:

1. **PaddleOCR-VL 1.6 for rejected complex/scanned pages and difficult tables.**
   This is the strongest new parser candidate: it is compact, permissively
   licensed, structured, and practical on both the local 16 GB GPU and the
   proposed AWS L4. It must be tested as the complete layout-plus-recognition
   pipeline, not as the standalone 0.9B recognition model.
2. **MinerU Hybrid `medium` as a second MinerU benchmark lane.** ORExtractor
...
3. **A persistent, corpus-wide visual work queue backed by a batched model
...
|---|---|---|---|---|
| **PaddleOCR-VL 1.6** | The official pipeline performs whole-page layout detection/reading order, crops elements, then recognizes text, tables, formulas, charts, seals, and spotting output. It exposes JSON/Markdown, client concurrency, and vLLM/SGLang/FastDeploy servers. Paddle explicitly warns that directly serving only the 0.9B VLM is **not** the complete pipeline and may not reproduce pipeline quality. The project reports 96.3% on OmniDocBench v1.6, but this is vendor-reported and not an NI 43-101 evaluation. [Pipeline documentation](https://github.com/PaddlePaddle/PaddleOCR/blob/main/docs/version3.x/pipeline_usage/PaddleOCR-VL.en.md), [official model card](https://huggingface.co/PaddlePaddle/PaddleOCR-VL-1.6) | Code and model are Apache-2.0. [PaddleOCR license](https://github.com/PaddlePaddle/PaddleOCR/blob/main/LICENSE), [model card](https://huggingface.co/PaddlePaddle/PaddleOCR-VL-1.6) | The official docs support x64 CPU and NVIDIA GPUs, include tuning guidance as low as RTX 3060, and provide CUDA 12.6 Docker images. vLLM/SGLang do not run natively on Windows in this deployment path; Docker/WSL or AWS Linux is required. No official ORExtractor-like pages/s figure was found. | **High-value bounded benchmark.** Add only as a page/window fallback at first. Preserve Bedrock/local Qwen semantic interpretation for geological maps and sections. |
...
- PaddleOCR-VL's 96.3% and MinerU's 0.13-point delta are vendor benchmark
  results, not evidence for numeric fidelity, page citations, NI Item tagging,
  or geological-figure handling.
...
3. Native-text preflight + PaddleOCR-VL 1.6 only for rejected page windows.
4. Current parser output + persistent vLLM Qwen3-VL server, compared with the
...
most likely useful first subsystem is **PaddleOCR-VL as a rejected-page adapter**;
the most likely useful performance subsystem is **persistent batched visual
serving without Ray**.
